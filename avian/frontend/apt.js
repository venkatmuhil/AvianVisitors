(function () {
  ...
  // ?v= on /avian/api/cutout.php and the bundled WebP art paths (narrow
  // one-species corrections use ART_REVISIONS below instead, so a single
  // fix doesn't evict the whole image cache).
  //
  ...
  var IMG_VERSION = 'r52';
  // Separate from the library-wide versions above: the ?v= on dims.json,
  // masks.json and style-overrides.json - the tables that hold upstream's
  // illustration set plus our photo-cutout-only masks for the Karnataka
  // species that have no bundled illustration (1231+ entries between them).
  // A table update should not evict every bird image, and a corrected image
  // should not evict the tables. bump_versions() in species-sync bumps this
  // too whenever it writes to dims.json/masks.json.
  var TABLE_VERSION = 'r13';
  // One-off art corrections, keyed by slug - consulted ahead of
  // SKETCH_VERSION/IMG_VERSION so a single fix doesn't force a whole-library
  // cache purge.
  var ART_REVISIONS = {
  ...
  // the admin gate on forwarded deploys, so a public visitor gets a 401.
  ...
  function artSrc(sci, pose, commonName) {
    ...
    if (tablesReady && DIMS[key]) {
      ...
      return './avian/assets/webp/' + key + '.webp?v=' + artRevision(sci, IMG_VERSION);
    }
  ...
  function rollPose(hasFlight) {
    return (hasFlight && Math.random() < FLY_PROB) ? 2 : 1;
  }
  ...
  function updateCollagePoses(items, allowReroll) {
    ...
    items.forEach(function (s) {
      ...
      if (!rec) {
        collagePose[s.sci] = { pose: rollPose(hasFlight), exp: poseExpiry(), pinned: false };
        return;
      }
      // Perched-only species never enter the candidate list, so they can
      // never churn the layout for a pose that does not exist.
      if (rec.pinned || !hasFlight) return;
      if (rec.exp <= now) due.push({ hasFlight: hasFlight, rec: rec });
    });
    ...
    for (var i = 0; i < due.length; i++) {
      var next = rollPose(due[i].hasFlight);
  ...
  function renderCollage(items, animate, reroll) {
    ...
    if (labelsOn() && !labelFontReady) { setTimeout(function () { renderCollage(items, animate, reroll); }, 60); return; }
    ...
    // This also owns the departure cleanup that used to trail the map.
    ...
    var tiles = items.map(function (s) {
      ...
      if (pose === 2 && !hasFlight) { pose = 1; if (rec) { rec.pose = 1; rec.pinned = true; } }
